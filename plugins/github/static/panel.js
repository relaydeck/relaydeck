...
// most rules care about. https://docs.github.com/en/webhooks/webhook-events-and-payloads
const EVENT_TYPES = [
...
const ACTION_TYPES = ['agent.message', 'script', 'gh', 'bus.emit'];

const ACTION_HINTS = {
  'agent.message': 'Send a peer message. {{ payload.path }} substitutions allowed in body.',
...
export default class GithubPanel {
  ...
  sections() {
    ...
    return [
      { group: 'GitHub', items: [
        { id: 'overview', label: 'Overview', icon: 'git', badge: wss.length || '' },
        { id: 'workspaces', label: 'Workspaces', icon: 'layers', badge: configured || '' },
  ...
  _gotoWorkspace(ws) {
    this.focusWs = ws;
    this._section = 'workspaces';
    this._ctx?.setSection?.('workspaces');
    this._openEditor(ws);
  }
  ...
  _card(w) {
    ...
    const repoCell = w.repo
      ...
      : html`<span class="repo missing">— no github.yaml —</span>`;
    ...
    const cursorErrCell = w.cursor.last_error
      ? html`<span class="val err" title=${w.cursor.last_error}>${w.cursor.last_error.slice(0, 120)}</span>`
      : (w.config_error
          ? html`<span class="val err" title=${w.config_error}>${w.config_error}</span>`
          : html`<span class="val muted">none</span>`);

    const rulesBlock = w.config_error
      ? html`<div class="err">${w.config_error}</div>`
      : (w.rules.length
          ? w.rules.map(r => html`
            <div class="row">
  ...
  _editor(ws) {
    const path = this.editorPath || `~/.relaydeck/workspaces/${ws}/github.yaml`;
    ...
    return html`
      <div class="gh-editor">
          ...
          <span>
            ...
            <span class="gh-editor-tabs">
              ...
              <button class=${this.editorMode === 'yaml' ? 'active' : ''}
                @click=${() => this._switchMode('yaml')}>YAML</button>
            </span>
          </span>
        ...
        <div class="actions">
          ...
          <button class="cancel" @click=${() => this._closeEditor()}>Cancel</button>
          <span class="msg ${msg ? msg.kind : ''}">${msg ? msg.text : ''}</span>
          <span style="color:var(--t-4);font-size:var(--t-xxs);margin-left:auto">${this.editorMode === 'yaml' ? '⌘/Ctrl+Enter to save · ' : ''}Esc to close</span>
        </div>
      </div>`;
  }
  ...
  _setAction(rule, value) {
    ...
    // No re-render: the action <select> is a leaf control; mutating the
    // model is enough and re-rendering would steal focus mid-pick.
  }
    ...
    if (kind === 'agent.message') {
      fields = html`
        ...
        <textarea placeholder="message body — use {{ payload.path }} for substitution"
          .value=${params.body || ''}
          @input=${e => this._setActField(rule, idx, kind, 'body', e.target.value)}></textarea>`;
    } else if (kind === 'script') {
      fields = html`
        <div class="lbl">path</div>
        <input type="text" .value=${params.path || ''} placeholder="scripts/notify.py (relative to workspace)"
          @input=${e => this._setActField(rule, idx, kind, 'path', e.target.value)}>
      ...
      fields = html`
        ...
        <input type="text" .value=${args} placeholder='pr comment {{ pull_request.number }} --body "ack"'
          @input=${e => this._setActField(rule, idx, kind, 'args', e.target.value)}>
        <div class="lbl">timeout</div>
        <input type="number" .value=${params.timeout || ''} placeholder="60" min="1"
          @input=${e => this._setActField(rule, idx, kind, 'timeout', e.target.value)}>`;
      ...
      fields = html`
        ...
        <input type="text" .value=${data} placeholder='{"key": "value"} — JSON object'
          @input=${e => this._setActField(rule, idx, kind, 'data', e.target.value)}>`;
    }
  ...
  async _openEditor(ws) {
    if (this.editing === ws) { this._closeEditor(); return; }
    this.editing = ws;
    this.editorMode = 'form';
    this.editorText = '';
    this.editorParsed = null;
    ...
    try {
      ...
      if (data.exists) {
        ...
        if (data.parsed) {
          ...
          if (data.error) this.editorMsg = {kind: 'err', text: data.error};
        }
      } else {
  ...
  _switchMode(mode) {
    ...
    if (mode === 'yaml') {
      ...
      if (!this.editorParsed) {
        this.editorMsg = {kind: 'err', text: 'YAML must parse cleanly before switching to form view'};
        this._render();
        return;
      }
      this.editorMode = 'form';
  ...
  async _saveConfigPayload(ws, payload, {editor = false} = {}) {
    try {
      ...
      if (!data.ok) {
        if (editor) {
          this.editorMsg = {kind: 'err', text: data.error || `HTTP ${r.status}`};
          this._render();
        } else {
          this._setToast(ws, data.error || `HTTP ${r.status}`, true);
        }
        return;
      }
      ...
      if (editor) {
        this.editorMsg = {kind: 'err', text: `${e.message || e}`};
        this._render();
      } else {
